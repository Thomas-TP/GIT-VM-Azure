...
export function RequestDetail() {
  ...
  const courseDefs = (r.course ?? '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
    .map((id) => cat?.courses.find((c) => c.id === id))
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Card className="p-5">
          ...
          <div className="divide-y divide-border">
            ...
            <Row label={t('detail.instanceType')} mono>{perfDef?.size ?? r.preset}</Row>
            ...
            {courseDefs.length > 0 && <Row label={t('newvm.course')}>{courseDefs.map((c) => c.label).join(', ')}</Row>}
          ...
          {courseDefs.length > 0 && (
            <div className="mt-3 border-t border-border pt-3">
              ...
              <div className="flex flex-wrap gap-1">
                {[...new Set(courseDefs.flatMap((c) => c.tools))].map((tool) => (
